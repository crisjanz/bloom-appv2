...
const FrequentlySoldProducts = () => {
  ...
  useEffect(() => {
    async function loadProducts() {
      try {
        const response = await api.get('/products/frequently-sold?limit=8');
        setProducts(response || []);
      } catch (error) {
        console.error('Failed to load frequently sold products:', error);
      } finally {
        setLoading(false);
      }
    }
      ...
      <section className="dark:bg-dark">
        <div className="container mx-auto overflow-hidden py-20">
          <div className="text-center">
            <p className="text-body-color dark:text-dark-6">Loading popular products...</p>
          </div>
        </div>
      </section>
    );
          ...
          >
            {products.map((product) => {
              ...
              const displayPrice = defaultVariant
                ? (product.basePrice + defaultVariant.priceDifference) / 100
...
export default FrequentlySoldProducts;
