...
model Order {
  ...
  // FTD wire order links
  ftdOrders     FtdOrder[]     @relation("FtdLinkedOrder")
...
model Discount {
  ...
  value           Float         // 20 (for 20% or $20), or sale price amount
  ...
  minimumOrder    Float?        // Minimum order amount required
...
model DiscountUsage {
  ...
  employeeId  String?  // Who applied it (POS context)
  source      String   // "POS", "WEBSITE"
  appliedValue Float   // Actual discount amount applied
  usedAt      DateTime @default(now())
...
model Event {
  ...
  quotedAmount    Float?      // Total quoted amount
  finalAmount     Float?      // Final agreed amount (may differ from quote)
...
model FtdOrder {
  ...
  postalCode        String?
  country           String        @default("CA")
  addressType       AddressType?  // Map FTD "Residence"→RESIDENCE, "Business"→BUSINESS
  ...
  cardMessage       String?
  occasion          String?
  productDescription String?       // "Vase Arrangement with Bear and Chocolates"
  productCode       String?        // "ARR"

  // === PRICING ===
  totalAmount       Float?

  // === FTD REPORTING ===
  sendingFloristCode String?       // For reports (eros_details.sender)
  ...
  ftdRawData        Json?          // Full FTD response
