...
const formatSegmentLabel = (segment: string, pathSegments: string[], index: number): string => {
  // Context-aware label mapping
  const previousSegment = index > 0 ? pathSegments[index - 1] : '';

  // Handle context-specific labels
  if (segment === 'edit') {
    if (previousSegment === 'products') return 'Edit Product';
    if (previousSegment === 'orders') return 'Edit Order';
    if (previousSegment === 'customers') return 'Edit Customer';
    return 'Edit';
  }

  if (segment === 'new') {
    if (previousSegment === 'products') return 'New Product';
    if (previousSegment === 'orders') return 'New Order';
    if (previousSegment === 'customers') return 'New Customer';
    return 'New';
  }
