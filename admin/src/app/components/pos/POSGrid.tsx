...
export default function POSGrid({ onAddProduct, onShowCustomModal, onDeliveryOrder, onScanOrder }: Props) {
  const { 
    products, 
      ...
      <div className="px-6 pb-6 flex-shrink-0">
        <div className="flex items-center justify-between gap-4">
          ...
          {onScanOrder && (
            <button
              onClick={onScanOrder}
              className="px-4 py-2.5 rounded-lg font-medium text-sm bg-brand-500 hover:bg-brand-600 text-white shadow-md transition-all duration-200 flex items-center gap-2 flex-shrink-0"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
