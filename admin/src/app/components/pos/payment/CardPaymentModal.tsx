...
export default function CardPaymentModal({ open, total, cardType, orderIds, customerEmail, customerPhone, customerName, onComplete, onCancel }: Props) {
  const [isProcessing, setIsProcessing] = useState(false);
  ...
  useEffect(() => {
    if (open) {
      ...
      setProvider('square'); // Default to Square for terminal
  ...
  }, [open, customerPhone, customerEmail]);
