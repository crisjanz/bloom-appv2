...
/**
 * Parse FTD order image using Google Gemini Vision
 */
export async function parseOrderImage(imageBuffer: Buffer): Promise<ParsedOrderData> {
  ...
  const prompt = `You are parsing a printed FTD wire service order. Extract the following information into a structured JSON format.
