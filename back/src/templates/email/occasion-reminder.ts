export interface OccasionReminderTemplateData {
  firstName: string;
  occasion: string;
...
export function buildOccasionReminderEmail(data: OccasionReminderTemplateData): string {
  const timing = data.daysBefore === 1
    ? "tomorrow"
    : `in ${data.daysBefore} days`;
  const recipientText = data.recipientName?.trim() ? ` for ${data.recipientName}` : "";
      ...
      <div style="max-width:600px; margin:0 auto; background:#ffffff; border-radius:14px; overflow:hidden;">
        ...
        <div style="padding:12px 28px 28px; color:#1f2937;">
          ...
          <p style="margin:0 0 20px; font-size:16px; line-height:1.6;">
            Your ${data.occasion.toLowerCase()} reminder${recipientText} is coming up ${timing}.
          </p>
          ...
          <p style="margin:0; font-size:12px; color:#6b7280;">
            Prefer not to receive this reminder?
